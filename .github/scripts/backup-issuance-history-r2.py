...
Usage:
  python .github/scripts/backup-issuance-history-r2.py [backup-file.json]
...
# Extract timestamp from filename: issuance_history-20251216T095000Z.json
# New structure: issuance/YYYY/MM/DD/HHMMSS.json
basename = os.path.basename(filepath)
try:
    # Parse timestamp from filename
    if 'issuance_history-' in basename:
        ts_part = basename.replace('issuance_history-', '').replace('.json', '')
    else:
    ...
    key_name = f"issuance/{year}/{month}/{day}/{time}.json"
    if R2_PREFIX:
