#!/usr/bin/env python3
"""
Upload a specified network history backup file to an S3-compatible R2 bucket.
...
Usage:
  python .github/scripts/backup-network-history-r2.py [backup-file.json]

If no file argument is provided, the script will look for the latest
`network_history-*.json` file in the current directory.
"""
import os
import sys
...
if len(sys.argv) > 1:
    ...
    # find latest network_history-*.json
    files = glob('network_history-*.json')
    if not files:
        print('No network_history-*.json files found to upload.')
        sys.exit(0)
