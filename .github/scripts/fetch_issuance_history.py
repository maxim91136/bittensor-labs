...
`upload_backup_r2.py` (the upload script is opt-in and controlled by
`ENABLE_R2`).
...
Environment variables used (all optional; script will skip if required CF vars missing):
  ...
  ENABLE_R2                If 'true', upload to R2 using upload_backup_r2.py
...
try:
    import subprocess
    subprocess.run(['python', '.github/scripts/upload_backup_r2.py', filename], check=False)
except Exception as e:
