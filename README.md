...
- A new scheduled workflow `backup-taostats-r2` runs every 3 hours and will fetch `taostats_history` from KV and upload a timestamped copy to R2. To enable the R2 upload set `ENABLE_R2=true` and the R2 credentials mentioned above.
 ...
 - A new per-run archival step in the `publish-taostats` workflow optionally uploads a timestamped entry to R2 for each fetch (if `ENABLE_R2=true`), so we can retain as much history as possible in R2 while keeping the recent history compact in KV. This ensures the dashboard can display long-term history later even if KV has size limits.
 - A daily consolidation job aggregates yesterday's raw per-run `taostats_entry-` files into a daily file `taostats_history-daily-YYYY-MM-DD.json` stored in R2, and deletes raw files older than a retention window (default 90 days). This is implemented by the new `consolidate-taostats-r2` workflow and script.
	- Note: the consolidation script uses the Cloudflare HTTP API if you supply `CF_API_TOKEN` and `CF_ACCOUNT_ID` (token must include R2 read/list/delete permissions). If your token does not include these permissions, you can alternatively provide S3-compatible R2 credentials in `R2_ENDPOINT`, `R2_ACCESS_KEY_ID`, and `R2_SECRET_ACCESS_KEY` (as repository secrets) and the consolidation script will use the S3 API instead.
