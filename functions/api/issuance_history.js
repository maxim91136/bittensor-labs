...
function getChunkKey(date = new Date()) {
  ...
  return `issuance_history_${yyyy}-${mm}-${dd}`;
}
...
export async function onRequest(context) {
  ...
  try {
    if (context.request.method === 'GET') {
      ...
      if (!usedChunks) {
        const legacyRaw = await KV.get('issuance_history');
        if (legacyRaw) {
      ...
      // Sort by ts (unix timestamp) and deduplicate
      const seen = new Set();
      combined = combined
        .filter(e => {
          const k = e?.ts || JSON.stringify(e);
    ...
    if (context.request.method === 'POST') {
      ...
      // Merge with deduplication (by ts)
      const seen = new Set(current.map(e => e?.ts || JSON.stringify(e)));
      for (const e of newEntries) {
        const k = e?.ts || JSON.stringify(e);
      ...
      try {
        ...
        const legacySeen = new Set(legacy.map(e => e?.ts || JSON.stringify(e)));
        for (const e of newEntries) {
          const k = e?.ts || JSON.stringify(e);
          if (legacySeen.has(k)) continue;
          legacy.push(e);
        }
        legacy.sort((a, b) => (a.ts || 0) - (b.ts || 0));
        ...
        await KV.put('issuance_history', JSON.stringify(legacy));
      } catch (e) {
